...
class GraphManager:
    def __init__(self, neo4j_url=None, neo4j_user=None, neo4j_password=None, db_path=DEFAULT_DB):
    ...
    def add_node(self, node_id, label, ntype='Event', timestamp=None, metadata=None, provenance=None):
        ...
        if self.use_neo4j:
            with self.driver.session() as s:
                s.run("""MERGE (n:Node {id:$id}) SET n.label=$label, n.type=$type, n.timestamp=$ts, n.metadata=$meta, n.provenance=$prov""", 
                      id=node_id, label=label, type=ntype, ts=timestamp, meta=json.dumps(metadata), prov=json.dumps(provenance))
        else:
    ...
    def add_edge(self, src_id, dst_id, etype='RELATES_TO', properties=None):
        ...
        if self.use_neo4j:
            with self.driver.session() as s:
                s.run("""MATCH (a:Node {id:$src}), (b:Node {id:$dst}) MERGE (a)-[r:REL {type:$t}]->(b) SET r.props=$props""", 
                      src=src_id, dst=dst_id, t=etype, props=json.dumps(properties))
        else:
    ...
    def _persist_graph(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute('REPLACE INTO kv (k,v) VALUES (?,?)', ('graph', json.dumps(gdata)))
            conn.commit()
